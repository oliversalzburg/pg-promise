...
// TODO: Needs proper reviewing and testing;
// TODO: Address the issue with a dummy init function;
function $stream(ctx, qs, init) {
    if (typeof(init) !== 'function') {
        return $p.reject("Stream initialization callback is missing.");
    }
    ...
    if (!stream._reading) {
        // stream wasn't initialized;
        return $p.reject("Stream not initialized.");
    }
...
function $query(ctx, query, values, qrm) {
    ...
    // TODO: Needs proper check that the object is of type `QueryStream`?
...
function $main(options) {
    ...
    // library version;
    inst.version = new Version(2, 0, 2);
