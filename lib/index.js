...
function $stream(ctx, qs, init) {
    ...
    if (qs.state !== 'initialized') {
        return $p.reject("Invalid state of the stream object.");
    }
    if (typeof(init) !== 'function') {
        return $p.reject("Invalid stream initialization callback.");
    }
    // TODO: qs.text - the sql query to be reported;
    var stream, fetch, start = Date.now(), count = 0;
    try {
    ...
    if (!stream._reading) {
        ...
        return $p.reject("Stream wasn't initialized by the callback function.");
    }
    return $p(function (resolve, reject) {
        stream.once('end', function () {
            // TODO: query notification?
            restore();
            resolve({
...
function $main(options) {
    ...
    // library version;
    inst.version = new Version(2, 0, 3);
