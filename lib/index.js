...
function $stream(ctx, qs, init) {
    ...
    var stream, oldFetch, start, nRows = 0;
    try {
        stream = ctx.db.client.query(qs);
        oldFetch = stream._fetch;
        var newFetch = function(size, func) {
            oldFetch.call(stream, size, function (err, rows) {
                nRows += err ? 0 : rows.length;
                return func(err, rows);
            });
        };
        stream._fetch = newFetch;
        start = Date.now();
    ...
    function restore() {
        if (stream._fetch === newFetch) {
            stream._fetch = oldFetch;
        }
    }
}
